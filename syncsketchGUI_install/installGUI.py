...
class InstallDialog(QWidget, UIDesktop):
    ...
    def clean(self):
        ...
        # del Ressources.GIFDEVICE #TODO figure out why this was used in first place
    ...
    def get_install_directory(self):
        return ScriptInstallPath[platform.system()]
    ...
    def _create_main_layout(self):
        ...
        company_logo = self._create_company_logo()
        main_layout.addWidget(company_logo, 0)

        subtext_label = self._create_subtext_label()
        ...
        main_layout.addLayout(subLayout2, 0)

        option_layout = self._create_option_layout()
        main_layout.addLayout(option_layout)

        button_layout = self._create_button_layout()
        main_layout.addLayout(button_layout)

        progress_layout = self._create_progress_layout()
    ...
    def _create_subtext_label(self):
        subtext = QLabel(
            u'Update Available: Would you like to upgrade to the latest?' if InstallOptions.upgrade else 'SyncSketch Integration for Maya [{} cut]'.format(
                versionTag))
    ...
    def _create_wait_label(self):
        wait_label = QLabel()
        wait_label.setText('Installing, please wait ...')
        return wait_label
    ...
    def _create_progress_layout(self):
        ...
        progressLayout.addWidget(self.wait_label)

        progressLayout.addWidget(self.animated_gif)
        self.animated_gif.hide()
        self.wait_label.hide()
    ...
    def _create_animated_gif(self):
        movie = QMovie()
        movie.setDevice(Ressources.GIFDEVICE)
        animated_gif = QLabel()
        animated_gif.setMovie(movie)
        animated_gif.setMaximumHeight(24)
        animated_gif.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        animated_gif.setScaledContents(True)
